...
const addProduct = async (req, res) => {
    ...
    Product.findOne({title: req.body.title, size: req.body.size});
    if (existProduct) {
        return res.status(400)
        .send({message: `Product '${existProduct.title}' 
        has declared ! Change title or size of product`});
    }
    try {
...
const editProduct = async (req, res) => {
    if (!isValid(req.params.id)) { return res.status(400).send('Such product not found !'); }
    try {
        const changedProduct = await 
